...
import { EstimateSchema } from '../models/estimatesModel';
...
export const updateEstimate = (req, res) => {
    Estimate.findOneAndUpdate({ _id: req.params.estimateID}, req.body, { new: true, useFindAndModify: false }, (err, estimate) => {
        if (err) {
            res.send(err);
        }
        res.json(estimate);
